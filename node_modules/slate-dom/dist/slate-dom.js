(function (global, factory) {
	...
	function verifyDiffState(editor, textDiff) {
	  ...
	  return slate.Node.isText(nextNode) && nextNode.text.startsWith(diff.text);
	}
	...
	function normalizePoint(editor, point) {
	  ...
	  if (!slate.Node.isText(leaf)) {
	    return null;
	  }
	  var parentBlock = slate.Editor.above(editor, {
	    match: function match(n) {
	      return slate.Node.isElement(n) && slate.Editor.isBlock(editor, n);
	    },
	    at: path
	  });
	  if (!parentBlock) {
	...
	var withDOM = function withDOM(editor) {
	  ...
	  e.deleteBackward = function (unit) {
	    ...
	    if (e.selection && slate.Range.isCollapsed(e.selection)) {
	      var parentBlockEntry = slate.Editor.above(e, {
	        match: function match(n) {
	          return slate.Node.isElement(n) && slate.Editor.isBlock(e, n);
	        },
	        at: e.selection
	      });
	      if (parentBlockEntry) {
	  ...
	  e.apply = function (op) {
	    ...
	    if (pendingAction !== null && pendingAction !== void 0 && pendingAction.at) {
	      var at = slate.Location.isPoint(pendingAction === null || pendingAction === void 0 ? void 0 : pendingAction.at) ? transformPendingPoint(e, pendingAction.at, op) : transformPendingRange(e, pendingAction.at, op);
	...
	exports.closestShadowAware = closestShadowAware;
	exports.containsShadowAware = containsShadowAware;
